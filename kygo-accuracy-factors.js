...
class KygoAccuracyFactors extends HTMLElement {
  ...
  get _src() {
    return {
      ...
      bfskin:    { url: 'https://journals.plos.org/plosone/article?id=10.1371%2Fjournal.pone.0353261', label: 'Body fat and skin tone on calorie error: four watches vs a COSMED K5 metabolic system during cycling, n=58 (Kostrna 2026, PLOS ONE)' },
  ...
  get _asked() {
    return [
      ...
      { key: 'bodyfat', ic: 'body', grp: 'yes', title: 'Higher body fat', badges: [{ t: 'yes', label: 'Matters for calories' }], dev: ['watch'], chips: ['Calories'],
        num: '+69 kcal in 10 min',
        one: 'Three of four watches ran well high on calories, and the error grew as body fat rises. Skin tone made no difference.',
        study: 'n=58 Hispanic adults, a 10-minute cycling test against a COSMED K5 metabolic system. Average overestimate: Garmin Forerunner 955 +69 kcal, Samsung Galaxy Watch 5 +57, Apple Watch Series 8 +22, Fitbit Sense 2 +3 but with 13% of its readings implausible (over 4.5 times the true value). Error rose with body fat on every brand, p<0.01; skin tone had no significant effect. One short cycling protocol, so treat the size as indicative. Kostrna 2026.',
        brands: 'No manufacturer addresses body composition in its calorie accuracy guidance.',
        todo: 'Read workout calories as a high estimate, and more so the higher your body fat. Keep your weight current in the app, and track the trend rather than eating back the number.',
        src: 'bfskin' },
  ...
  _faqs() {
    return [
      ...
      { q: 'Does body fat affect smartwatch calorie accuracy?', a: 'Yes. Against a metabolic system during cycling, n=58, every watch tested overestimated calories and the error grew with body fat on every brand. Skin tone made no significant difference.' },
  ...
  _icon(name) {
    const icons = {
      ...
      body: '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="12" cy="5" r="2.5"/><path d="M7 21v-6.5a5 5 0 0 1 10 0V21"/><path d="M9.5 21v-4"/><path d="M14.5 21v-4"/></svg>',
  ...
  get _srcGroups() {
    ...
    return {
      ...
      'Body, physiology and clinical populations': [S.shch, S.adip, S.bfskin, S.fitmeta, S.aid, S.af, S.cvdhrv, S.sdb, S.osa],
  ...
  _seoText() {
    ...
    return 'Wearable accuracy: what actually affects it, by Kygo Health. Where you wear a tracker matters more than which one you bought. ' +
      ...
      'Conditions the sensor or the model was never built for: rowing 13.4% against 3.8% walking on the same device, swimming 29.95% wet against 4.05% dry, no wrist device acceptable on an elliptical with arm levers at n=50; resistance-training calories read 116% high against indirect calorimetry while heart rate correlated 0.96 to 0.97, n=62; body fat: in a 10-minute cycling test against a COSMED K5, n=58, Garmin Forerunner 955 overestimated by 69 kcal, Samsung Galaxy Watch 5 by 57, Apple Watch Series 8 by 22 and Fitbit Sense 2 by 3 with 13% implausible readings, and calorie error rose with body fat on every brand while skin tone had no significant effect (Kostrna 2026, PLOS ONE); walking below about 4 km/h gives 40% error against 7% at normal pace, n=258 across 21 devices, and stairs failed the 10% threshold in every condition tested; the first night with a new device is not a baseline and it takes about seven nights for a stable personal mean; battery-saver modes switch background heart rate and blood oxygen off entirely, which looks like non-wear rather than error; missing nights rose from 22% of participants on night one to 47% by night five, n=299. ' +
      'Tested and it changed nothing: arm hair, which wrist for heart rate and sleep, wearing two optical devices at once (biases -0.05 to +2.93 bpm, no missing values, n=16), and skin tone on average bias, for heart rate and for calories. Never tested at all: shaving on its own, lotion and sunscreen at a wearable site, sharing a bed with a partner or a pet, wrist size on current hardware, which finger a ring goes on, and device age across 249 validation studies and 430,465 participants. ' +
  ...
  _appCta() {
    return {
      ...
      hook: `Find out which of these is actually skewing your own numbers, night after night.`,
