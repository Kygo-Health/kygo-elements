...
class KygoSmartRingComparison extends HTMLElement {
  ...
  get _models() {
    return [
      ...
      { key: 'cudiss', brand: 'cudis', name: 'CUDIS 002 Sporty', short: '002 Sporty', hw: 399, sub: 0, addon: 0, battery: 10,
        ...
        why: 'The Classic plus 12 interchangeable silicone bands and a Sports Mode. Same sensor set, same missing temperature sensor, same absent accuracy record.',
        s: { nosub: 100, evidence: 5, apnea: 0, women: 20, usnow: 100 } },
    ];
  }
      ...
      <section class="section bg-light">
        <div class="section-inner">
          <div class="section-head animate-on-scroll">
            ...
            <h2>All four publish claims. <span class="hl">Only one has been checked.</span></h2>
            <p class="lede">No ring here is a medical device and none holds an FDA clearance of its own. The difference between these brands is not the sensor, it is who has verified the output. Read every accuracy number with the type of evidence behind it in mind.</p>
          </div>
  ...
  _renderTLDR() {
    return `
      ...
      <div class="cmp-blocks">
        <div class="cmp-block">
          <h3>Oura</h3>
          <p class="cmp-verdict">The deepest software, and the only checked accuracy.</p>
        ...
        <div class="cmp-block">
          <h3>RingConn</h3>
          <p class="cmp-verdict">The most ring for the money.</p>
        ...
        <div class="cmp-block">
          <h3>Ultrahuman</h3>
          <p class="cmp-verdict">The biggest ecosystem, the smallest evidence base.</p>
        ...
        <div class="cmp-block">
          <h3>CUDIS</h3>
          <p class="cmp-verdict">Cheap, unproven, and missing a sensor.</p>
          <p>$349 or $399 with no subscription and a battery that beats its own claim. It publishes no engineering spec, no accuracy figure, and it has no temperature sensor, so temperature-derived cycle features are off the table.</p>
        </div>
        <div class="cmp-block cmp-block-wide">
          <h3>So which should you buy?</h3>
          <p class="cmp-verdict">RingConn Gen 3 for most people, Oura if the evidence matters most.</p>
          <p>If you want the most capability per dollar with nothing recurring, the Gen 3 is the straightforward answer. If you want a number you can trust and a software platform that keeps moving, Oura earns its fee. Take the Ring PRO if battery and the wider Ultrahuman stack are the point and you can wait out the US pre-order. Take CUDIS only if $349 with no subscription is the whole brief and you accept that nothing about its accuracy has been published.</p>
        </div>
      </div>
    `;
  }
  ...
  _renderCalc() {
    ...
    const gapMsg = gap === 0
      ...
      : `<strong>${dearer.name}</strong> costs <strong>${fmt(Math.abs(gap))} more</strong> than ${cheaper.name} over ${years} ${years === 1 ? 'year' : 'years'}.`
        ...
        + ` Cost is one input: check what each side actually publishes in the feature and validation tables above before letting the dollar gap decide.`;
  ...
  get _validation() {
    return [
      { brand: 'oura', title: "Oura's posture", tag: 'Independent peer-reviewed studies', body: 'The only brand here that points to peer-reviewed validation studies it did not run, plus a medical advisory board. It positions itself as research-grade and claims a pulse signal up to 100x stronger than a wrist device. Commercial signals like roughly 5M paid members and a 2026 IPO filing are scale, not evidence.' },
  ...
  get _caveats() {
    return [
      { icon: 'alert', title: 'Two Ultrahuman rings cannot legally ship to a US address', body: 'In ITC investigation 337-TA-1398 the Commission found Ultrahuman infringed Oura\'s US Patent 11,868,178 and issued a limited exclusion order plus a cease-and-desist order in August 2025, with the bond set at zero percent. RingConn settled and took a royalty-bearing licence; Ultrahuman did not, and its Federal Circuit appeal is still undecided. Only the redesigned Ring PRO enters the US, under CBP ruling HQ H354023 of March 6, 2026, because its unibody titanium tube falls outside the patent claims. The Ring AIR, the Diesel edition and the $1,799 Ring Rare are all blocked.', src: 'Federal Register · CBP CROSS ruling HQ H354023' },
  ...
  _styles() {
    return `
      ...
      * { box-sizing: border-box; }
      ...
      .finder-note { margin: 4px 0 0; font-size: 12.5px; line-height: 1.6; color: var(--fg-3); max-width: 90ch; }
