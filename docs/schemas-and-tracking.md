...
| `kygo-accuracy-factors.js` | `data-kygo-accuracy-factors-ld`, `data-kygo-accuracy-factors-faq`, `data-kygo-accuracy-factors-bc` | WebApplication, FAQPage (8 Q&A, from the visible `_faqs()` getter), BreadcrumbList, Offer, Organization. WebApplication `name` = "Wearable Accuracy Factor Explorer", `url` = `/tools/accuracy-factors`. |
