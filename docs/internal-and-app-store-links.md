...
directly above the related-reading section, which in turn sits above sources.
One card definition per destination, reused wherever that tool is linked. **The Food Scanner (`/tools/calories-in-anything`) is
...
- **Posts linked from tool pages now go through the standard related-reading module**
  (`_relatedPosts()` + `_renderRelatedPosts(bg)`, 3 cards per page, directly below related
  tools). The full map — which tool links which posts, inbound counts, and the card copy for
